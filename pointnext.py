...
class SetAbstraction(nn.Module):
    ...
    def forward(self, points_coor, points_fea):
        ...
        '''S 采样'''
        new_coor = index_points(points_coor, farthest_point_sample(points_coor, num_point_out)) 
        ...
        grouped_points_coor = index_points(points_coor[..., :3], group_idx) 
        grouped_points_coor -= new_coor[..., :3].view(bs, num_point_out, 1, 3)  
        grouped_points_coor = grouped_points_coor / self.radius  
        grouped_points_fea = index_points(points_fea, group_idx)  
        grouped_points_fea = torch.cat([grouped_points_fea, grouped_points_coor], dim=-1)  
...
class LocalAggregation(nn.Module):
    ...
    def forward(self, points_coor, points_fea):
        """
        :param points_coor: <torch.Tensor> (B, 3, N) 
        :param points_fea: <torch.Tensor> (B, C, N) 
        :return:

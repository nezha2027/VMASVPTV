...
class PCDPretreatment(nn.Module):
    ...
    def __init__(self, num=2048, padding=True, down_sample='random', mode='train', normal=True,
    ...
    def forward(self, pcd, gts):
        ...
        pcd_xyz /= max_dis
        # 与身长为标准的归一化
        # scale_rate = pcd_xyz.max(0)[0] - pcd_xyz.min(0)[0]
        # scale_rate = scale_rate.max()
        # pcd_xyz /= scale_rate
        ## 标签归一化
        scale_rate = gts.max(0)[0] - gts.min(0)[0]
        scale_rate = scale_rate.max()
        gts /= scale_rate
        ...
        if pcd.shape[0] < self.num and self.padding:
            ...
            pcd, downsample_ids = self.down_sample(pcd)

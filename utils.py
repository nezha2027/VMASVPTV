...
def coordinate_distance(src, dst):
    """
    :param src: <torch.Tensor> (B, M, C) 
    :param dst: <torch.Tensor> (B, N, C) 
...
def index_points(points, idx):
    ...
    :param idx: <torch.Tensor> (B, S)/(B, S, G) 
    :return: <torch.Tensor> (B, S, 3+)/(B, S, G, 3+) 
    """
    B = points.shape[0]
...
def query_hybrid(radius, nsample, xyz, new_xyz):
    ...
    dist = coordinate_distance(new_xyz, xyz)  
    dist, group_idx = torch.topk(dist, k=nsample, dim=-1, largest=False)  
    radius = radius ** 2
    mask = dist > radius  
